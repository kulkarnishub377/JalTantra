document.addEventListener('DOMContentLoaded', () => {
    ...
    async function fetchWeather() {
        try {
            ...
            if(heroTemp) heroTemp.innerText = `${Math.round(current.temperature)}°C`;
            ...
            if(heroCond) heroCond.innerText = conditions[current.weathercode] || 'Unknown';
            const heroWind = document.getElementById('hero-wind');
            if(heroWind) heroWind.innerText = `${current.windspeed} km/h`;
    ...
    async function callGemini(prompt, resId, btnId, useModal = false, imgBase64 = null) {
        ...
        if (!apiKey || apiKey === '__REPLACE_WITH_YOUR_API_KEY__') {
            ...
            if (useModal) {
                ...
                if(modalContent) modalContent.innerHTML = message;
    ...
    if(chatSendFloat) {
        chatSendFloat.addEventListener('click', () => {
    ...
    function updateTicker() {
        ...
        const mandiTicker = document.getElementById('mandi-ticker');
        if(mandiTicker) mandiTicker.innerHTML = h + h;
    }
